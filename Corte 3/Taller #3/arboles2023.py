...
class ArbolBinarioBusqueda:
    ...
    def crearArbolParcialmenteOrdenado(elementos):
        if not elementos:
            return None
        
        elementos_ordenados = sorted(elementos)
        return ArbolBinarioBusqueda.__crearArbolParcialmenteOrdenadoRecursivo(elementos_ordenados)

    def __crearArbolParcialmenteOrdenadoRecursivo(elementos):
        if not elementos:
            return None
        
        indice_central = len(elementos) // 2
        valor_central = elementos[indice_central]
        nodo = ArbolBinario(valor_central)

        nodo.hijo_izquierdo = ArbolBinarioBusqueda.__crearArbolParcialmenteOrdenadoRecursivo(elementos[:indice_central])
        nodo.hijo_derecho = ArbolBinarioBusqueda.__crearArbolParcialmenteOrdenadoRecursivo(elementos[indice_central + 1:])
    ...
    def esAVL(self):
        if self.raiz is None:
            return True
        return self.__esAVL(self.raiz)

    def __esAVL(self, nodo):
        if nodo is None:
            return True

        altura_izquierda = self.__altura(nodo.hijo_izquierdo)
        altura_derecha = self.__altura(nodo.hijo_derecho)
        factor_equilibrio = altura_izquierda - altura_derecha

        if factor_equilibrio < -1 or factor_equilibrio > 1:
            return False

        return self.__esAVL(nodo.hijo_izquierdo) and self.__esAVL(nodo.hijo_derecho)
    ...
    def insertarNodosListaFraccionarios(self, listaFraccionarios):
        for fraccion in listaFraccionarios:
            self.insertar(fraccion)

    def insertar(self, fraccion):
        if self.raiz is None:
            self.raiz = ArbolBinario(fraccion)
        else:
            self.__insertar(self.raiz, fraccion)

    def __insertar(self, arbol, fraccion):
        if fraccion < arbol.valor_nodo:
            if arbol.hijo_izquierdo is None:
                arbol.hijo_izquierdo = ArbolBinario(fraccion)
            else:
                self.__insertar(arbol.hijo_izquierdo, fraccion)
        else:
            if arbol.hijo_derecho is None:
                arbol.hijo_derecho = ArbolBinario(fraccion)
            else:
                self.__insertar(arbol.hijo_derecho, fraccion)
